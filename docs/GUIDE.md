...
Machine-verified on hardware 2026-08-06 (macOS 15.6.1, Apple Silicon) by driving the running daemon over its own WebSocket API: focus, enumeration, raise, and pointer deltas. Scroll, clicks, and key injection are implemented but **nobody has watched an injected event land in an app** — see the evidence table in [PLATFORM-PARITY.md](PLATFORM-PARITY.md#verification-status) for what's actually been observed versus what merely compiles.
