...
- MPRIS media browser (chrome view with per-player transport, album art proxy, passive playback indicator).
...
- macOS has no MPRIS equivalent — the MPRIS media browser and chrome indicator are Linux-only.
...
For the full wire shape, see `daemon/deckd/protocol.py` (daemon) and `client/src/protocol.ts` (client). The daemon's Pydantic models are the authoritative schema — they define every field, its type, and its constraints. The client mirrors them in TypeScript but never interprets action bodies.
